...
taxos_list=[]
# Calculamos taxonomía propuesta para distintos k (10, 30, 50) para el archivo resumen
for n in [10, 30, 50]:
    taxo_prop, pesos = tools.datos(bus_tax[1], n)
    taxos_list.append((taxo_prop, pesos))
...
# archivos.crea_res espera una lista FP con 3 entradas (k=10,30,50), cada una como (taxonomia, fp)
FP = []
for (taxo_prop, pesos) in taxos_list:
    fp = pesos[np.argmax(pesos)]/np.sum(pesos)
    FP.append((taxo_prop[np.argmax(pesos)], fp))

# Asegurar que exista el directorio de salida
os.makedirs(os.path.join(script_dir, 'output'), exist_ok=True)

# Crear resumen en output/summary_<archivo>
archivos.crea_res(archivo, FP, predicKNN, predicPCA, predicNCA)
